...
if (result.Success)
 ...
 Debug.Log($"云存档初始化11");
    ...
    private async Task<bool> InitializeStartupManagers(HashSet<Type> completedManagers)
    {
        ...
        try
        {
            startupManagers = ManagerStartupRegistry.SortStartupManagers(ManagerStartupRegistry.CreateStartupManagerRegistrations(), completedManagers);
        }
        catch (Exception e)
        {
            hideSave = true;
    ...
    private async Task<bool> WaitForStartupManager(StartupManagerRegistration registration, HashSet<Type> completedManagers)
    {
        try
        {
            ValidateStartupRegistrationDependencies(registration, completedManagers);
    ...
    private static void ValidateStartupRegistrationDependencies(StartupManagerRegistration registration, HashSet<Type> completedManagers)
    {
        for (int i = 0; i < registration.Dependencies.Count; i++)
        {
            ...
            {
                throw new InvalidOperationException($"Startup manager registry dependency is not completed: manager={registration.ManagerType.Name}, dependency={dependency.Name}");
            }
        }
    }
