...
public class ProFlareBatch : MonoBehaviour {
	...
	public bool cullingForRay2D = false;
...
//		Debug.LogError("CreateMat");
			...
			for (int F = 0; F < FlaresList.Count; F++) {
				...
				if (flare_.isVisible) {
					...
					if (flare_.RaycastPhysics) {
						...
						if (cullingForRay2D)
						{
                            Ray2D ray2D = new Ray2D(flare_.thisTransform.position, direction);
                            RaycastHit2D raycastHit2D = Physics2D.Raycast(ray2D.origin, ray2D.direction, distanceRay, flare_.mask);
							if (raycastHit2D.transform)
							{
								hitGameObject = raycastHit2D.transform.gameObject;
							}
                            rayHit=raycastHit2D.transform!=null;
                        }
						else
						{
                            Ray ray = new Ray(flare_.thisTransform.position, direction);
							rayHit=Physics.Raycast(ray, out hit, distanceRay, flare_.mask);
							if (rayHit)
						...
						if (rayHit) {
							...
							flare_.OccludingObject = hitGameObject;
							#endif
						} else {
							data.occluded = false;
	...
	public void SetTrigger2DGameObject(bool hole)
	{
		for (int F = 0; F < FlaresList.Count; F++)
		{
            if (FlaresList[F].flare == null)
            ...
            if (hole)
            {
				if (flare_.Elements.Count > 2)
				{
					flare_.Elements[2].Visible = false; 
                }
			...
			else
			{
                if (flare_.Elements.Count > 2)
                {
                    flare_.Elements[2].Visible = true;
                }
                //  data.occluded = false;
                //  data.occlusionTarget = 1;
...
//			Debug.Log("distanceFalloff "+distanceFalloff);
			...
			{
                LensPosition = lightTrans.localPosition;
            }
