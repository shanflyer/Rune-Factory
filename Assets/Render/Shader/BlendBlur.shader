               ...
               // sampler2D _MyBlurTex; 
                ...
                half4 frag(v2f IN) : SV_Target
                {
                   // uint2 pixelCoords = uint2(i.uv.xy * _ScreenSize.xy);
                    ...
                    Unity_Remap_float(x,float2(0.5+_BlurOffsetPos,1),_ReMapValue.xy,x);
                    x=clamp(x,0,1)*step(0.5-_BlurOffsetPos,myDepthColor.x);

                   


                    float x1=myDepthColor.x;
                    Unity_Remap_float(x1,float2(0.5-_BlurOffsetPos,0),_ReMapValue.xy,x1);
                    x1=clamp(x1,0,1)*(1-step(0.5-_BlurOffsetPos,myDepthColor.x));
                    x+=x1;
