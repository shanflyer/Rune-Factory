    ...
    SubShader
    {
        ...
        { 
            ...
            Varyings UnlitVertex(Attributes attributes)
            {
                ...
                LightDirection.x=(1-abs(_DirIndex))*LightDirection.x+clamp(LightDirection.x,0,90)*step(1,_DirIndex)+clamp(LightDirection.x,-90,0)*step(1,-_DirIndex);
                //LightDirection.y=2;
               ...
               // lightAngleValue=(1-abs(_DirIndex))*lightAngleValue+clamp(lightAngleValue,0,1)*step(1,_DirIndex)+clamp(lightAngleValue,-1,0)*step(1,-_DirIndex);
                ...
                //float ScaleValue=1-LightDirection.y;
                //Unity_Remap_float(ScaleValue,float(0,0.5))

                float2 offset=length.xx*float2(sin(LightDirection.x),cos(LightDirection.x))*(1-_Direction.y);

                worldPos.xy+=offset*attributes.uv2.y*(1-_SpriteShadow)+offset*_SpriteShadow;
                
                o.positionCS = TransformWorldToHClip(worldPos);
                attributes.color=attributes.color*(1-_SpriteShadow)+attributes.color*_SpriteShadow* unity_SpriteColor;
