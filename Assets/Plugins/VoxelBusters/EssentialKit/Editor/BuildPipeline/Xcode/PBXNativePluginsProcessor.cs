...
#region Base class methods
            ...
            foreach (var exporter in NativePluginsExporterObject.FindObjects<PBXNativePluginsExporterObject>(includeInactive: true))
            {
                switch (exporter.name)
                {
                    case NativeFeatureType.kAppUpdater:
                        ...
                        {
                            throw new Exception("[Essential Kit : Configuration Incomplete] App Store Id is not set in Essential kit settings inspector. Set it in the Essential Kit settings's General->Store Id's.");
                        }
                    break;
                }
            }
        ...
        public override void OnUpdateEntitlementsPlist(PlistDocument doc)
        {
            ...
            PlistElementString kvstoreEntryValue;

            if (rootDict.TryGetElement(kvstoreEntryKey, out kvstoreEntryValue))
            {
...
#region Private methods
        ...
        private string[] GeneratePBXBindingsFilesForUnusedFeatures(BuildTarget buildTarget)
        {
            ...
            var     generator           = new NativeBindingsGenerator(outputPath: outputPath, options: NativeBindingsGeneratorOptions.Source)
                ...
                .SetCopyrights("Copyright (c) 2024 Voxel Busters Interactive LLP. All rights reserved.");
            ...
            foreach (var item in ImplementationSchema.GetAllRuntimeConfigurations())
            {
                ...
                foreach (var bindingType in runtimePackage.GetBindingTypeReferences())
                {
                    ...
                    {
                        customTypes     = new Type[] { typeof(SKPaymentTransactionData), typeof(SKBuyProductOptionsData) };
                    }
                    else if (bindingType == typeof(AddressBookBinding))
                    {
                        customTypes     = new Type[] { typeof(NativeReadContactsOptionsData) };
                    }
                    else if (bindingType == typeof(MediaServicesBinding))
                    {
                        customTypes     = new Type[] { typeof(NativeMediaContentSelectionOptionsData), typeof(NativeMediaContentCaptureOptionsData), typeof(NativeMediaContentSaveOptionsData)};
                    }
                    else if (bindingType == typeof(AppUpdaterBinding))
                    {
                        customTypes     = new Type[] { typeof(NativeAppUpdaterPromptOptionsData)};
                    }
