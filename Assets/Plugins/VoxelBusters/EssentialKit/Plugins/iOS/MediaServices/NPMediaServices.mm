...
-(void) saveWithPhotoLibrary:(NSURL*) url mime:(NSString*) mime options:(MediaContentSaveOptions*) options callback:(SaveMediaContentCallback) callback{
    ...
    [self requestPhotoLibraryPermission:PHAccessLevelAddOnly withCallback:^(BOOL isAuthorised, NSError *error) {
        if(error != nil)
        {
            if(callback != nil)
            {
                callback(FALSE, [NPMediaServicesError createFrom:error]);
            }
            return;
        }
...
-(void) requestPhotoLibraryPermission:(PHAccessLevel) accessLevel withCallback:(void(^)(BOOL isAuthorised, NSError* error))callback
{
