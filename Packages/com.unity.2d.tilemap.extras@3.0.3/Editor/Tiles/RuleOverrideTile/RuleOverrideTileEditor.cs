...
namespace UnityEditor
{
    ...
    public class RuleOverrideTileEditor : Editor
    {
        ...
        public List<KeyValuePair<TileBase, TileBase>> m_TileBases = new List<KeyValuePair<TileBase, TileBase>>();
        /// <summary>
        ...
        private ReorderableList m_TileBaseList;
        private int m_MissingOriginalSpriteIndex;
        private int m_MissingOriginalBaseIndex;
        ...
        public virtual void OnEnable()
        {
            ...
            if (m_TileBaseList == null)
            {
                m_TileBaseList = new ReorderableList(m_TileBases, typeof(KeyValuePair<Sprite, Sprite>), false, true, false, false);
                m_TileBaseList.drawHeaderCallback = DrawGameObjectListHeader;
                m_TileBaseList.drawElementCallback = DrawTileBaseElement;
                m_TileBaseList.elementHeightCallback = GetTileBaseElementHeight;
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            overrideTile.GetOverrides(m_TileBases, ref m_MissingOriginalBaseIndex);
            ...
            EditorGUI.BeginChangeCheck();
            m_TileBaseList.DoLayoutList();
            if (EditorGUI.EndChangeCheck())
            {
                overrideTile.ApplyOverrides(m_TileBases);
                SaveTile();
            }
            EditorGUI.BeginChangeCheck();
        ...
        public float GetTileBaseElementHeight(int index)
        {
            float height = k_GameObjectElementHeight + k_PaddingBetweenRules;

            bool isMissing = index >= m_MissingOriginalBaseIndex;
            if (isMissing)
                height += 16;
        ...
        public void DrawTileBaseElement(Rect rect, int index, bool active, bool focused)
        {
            bool isMissing = index >= m_MissingOriginalBaseIndex;
            if (isMissing)
            {
            ...
            TileBase originalTileBase = m_TileBases[index].Key;
            TileBase overrideTileBase = m_TileBases[index].Value;
            ...
            rect.xMax = rect.xMax / 2.0f;
            using (new EditorGUI.DisabledScope(true))
                EditorGUI.ObjectField(new Rect(rect.xMin, rect.yMin, rect.width, rect.height), originalTileBase, typeof(TileBase), false);
            rect.xMin = rect.xMax;
            rect.xMax *= 2.0f;

            EditorGUI.BeginChangeCheck();
            overrideTileBase = EditorGUI.ObjectField(new Rect(rect.xMin, rect.yMin, rect.width, rect.height), overrideTileBase, typeof(TileBase), false) as TileBase;
            if (EditorGUI.EndChangeCheck())
                m_TileBases[index] = new KeyValuePair<TileBase, TileBase>(originalTileBase, overrideTileBase);
        }
        /// <summary>
