...
namespace UnityEngine.Tilemaps
{
    ...
    public class RuleOverrideTile : TileBase
    {
        ...
        {
            /// <summary>
            /// Original TileBase from the original RuleTile.
            /// </summary>
            public TileBase m_OriginalBase;
            /// <summary>
            /// Overriding Sprite for the Original TileBase.
            /// </summary>
            public TileBase m_OverrideBase;
        }
        /// <summary>
        ...
        public TileBase this[TileBase originalBase]
        {
            get
            {
                foreach (TileBasePair basePair in m_Bases)
                {
                    if (basePair.m_OriginalBase == originalBase)
                    {
                        return basePair.m_OverrideBase;
                    }
                }
                return null;
            }
            set
            {
                if (value == null)
                {
                    m_Bases = m_Bases.Where(basePair => basePair.m_OriginalBase != originalBase).ToList();
                }
                else
                {
                    foreach (TileBasePair basePair in m_Bases)
                    {
                        if (basePair.m_OriginalBase == originalBase)
                        {
                            basePair.m_OverrideBase = value;
                            return;
                        }
                    }
                    m_Bases.Add(new TileBasePair()
                    {
                        m_OriginalBase = originalBase,
                        m_OverrideBase = value,
                    });
                }
            }
        }
        ...
        public List<TileBasePair> m_Bases = new List<TileBasePair>();
        /// <summary>
        ...
        public void ApplyOverrides(IList<KeyValuePair<TileBase, TileBase>> overrides)
        {
            if (overrides == null)
                throw new ArgumentNullException("overrides");
        ...
        public void GetOverrides(List<KeyValuePair<TileBase, TileBase>> overrides, ref int validCount)
        {
            if (overrides == null)
                throw new ArgumentNullException("overrides");
            ...
            List<TileBase> originalTileBases = new List<TileBase>();
            ...
            if (m_Tile)
            {
               ...
               //    originalGameObjects.Add(m_Tile.m_DefaultGameObject);
                ...
                foreach (RuleTile.TilingRule rule in m_Tile.m_TilingRules)
                    if (rule.m_TileBase && !originalTileBases.Contains(rule.m_TileBase))
                        originalTileBases.Add(rule.m_TileBase);
            }

            validCount = originalTileBases.Count;

            foreach (var pair in m_Bases)
                if (!originalTileBases.Contains(pair.m_OriginalBase))
                    originalTileBases.Add(pair.m_OriginalBase);

            foreach (TileBase tileBase in originalTileBases)
                overrides.Add(new KeyValuePair<TileBase, TileBase>(tileBase, this[tileBase]));
        }
        ...
        public virtual void Override()
        {
            ...
            foreach (var rule in tile.m_TilingRules)
            {
                ...
                rule.m_TileBase = this[rule.m_TileBase] ?? rule.m_TileBase;
            }
        }
