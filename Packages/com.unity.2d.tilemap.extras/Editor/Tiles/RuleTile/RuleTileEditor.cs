...
namespace UnityEditor
{
    ...
    public class RuleTileEditor : Editor
    {
        ...
        /// </summary>
        public const float k_DefaultElementHeight = 82f;
        /// <summary>
        ...
        public float GetElementHeight(RuleTile.TilingRuleOutput rule)
        {
            ...
            switch (rule.m_Output)
            {
                ...
                case RuleTile.TilingRuleOutput.OutputSprite.Sequence:
                case RuleTile.TilingRuleOutput.OutputSprite.FixedSequeceSingle:
        ...
        public override void OnInspectorGUI()
        {
            ...
            tile.m_LayerOrder = EditorGUILayout.IntField(Styles.layerOrder, tile.m_LayerOrder);
            ...
            if (count == 0)
            {
                Rect rect = EditorGUILayout.GetControlRect(false, EditorGUIUtility.singleLineHeight * 6);
        ...
        {
            float y = rect.yMin;
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), Styles.tilingRulesType);
            tilingRule.m_RuleType = (RuleType)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), "", tilingRule.m_RuleType);
            ...
            y += k_SingleLineHeight;
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), Styles.tilingRulesTileBase);
            tilingRule.m_TileBase = (TileBase)EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_TileBase, typeof(TileBase), false);
            ...
            if (tilingRule.m_Output == RuleTile.TilingRuleOutput.OutputSprite.Sequence ||
               tilingRule.m_Output == RuleTile.TilingRuleOutput.OutputSprite.FixedSequeceSingle)
            {
                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), Styles.tilingRulesRow);
                tilingRule.row = EditorGUI.IntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.row);
                y += k_SingleLineHeight;

                GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), Styles.tilingRulesCol);
                tilingRule.col = EditorGUI.IntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.col);
                y += k_SingleLineHeight;
            }
